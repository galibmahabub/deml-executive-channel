...
place for that.
Go to github.com and click Sign up. Make a free account.
Once logged in, click the + icon top-right → New repository.
Name it something like `deml-executive-channel`. Leave it Public.
Don't check any extra boxes. Click Create repository.
On the next page, click the link that says uploading an existing file.
Drag these 3 items into the browser window:
`server.js`
`package.json`
...
Go to render.com and click Get Started. Sign up using your
GitHub account (easiest — one click, no new password).
On the Render dashboard, click New + → Web Service.
It will show your GitHub repos — click Connect next to the
...
When it's done, look near the top of the page for your URL — it'll look
like:
```
   https://deml-executive-channel.onrender.com
   ```
Open that URL in your browser. That's your live website.
Look at the Logs tab in Render — the President username/password get
